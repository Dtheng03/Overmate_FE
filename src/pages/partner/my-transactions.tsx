...
import { AddMoney } from "@/components/icons/commons";
...
function DashboardTransactions() {
    ...
    const { data, isFetching } = useQuery({
        queryKey: ['partner-transactions', pageNumber, debouncedSearchTerm, statusFilter],
        queryFn: () =>
            axiosClient.get(
                `/auth/transaction/service_owner?PageSize=${pageSize}&PageNumber=${pageNumber}&SearchTerm=${debouncedSearchTerm}&FilterStatus=${statusFilter}`
            ),
        staleTime: 3000,
        retry: 0,
    });
        ...
        <section className="min-h-[calc(100vh-58px)] p-[4%] bg-color1 bg-[url('./assets/imgs/background.png')] bg-cover bg-center">
            <div className={"my-2 p-0.5 rounded-[20px] bg-gradient-to-r from-[#011949] to-[#55A6CE]"}>
                <div className="bg-color1 p-2 rounded-[20px]">
                    <h1 className="text-center text-color4 text-lg font-bold uppercase">Giao dịch nạp tiền</h1>
                </div>
            </div>
            ...
            <div className="my-4 flex justify-between items-center">
                ...
                <Button
                    ...
                    onClick={() => {
                        navigate("/partner/new-transaction")
                    }}
                >
